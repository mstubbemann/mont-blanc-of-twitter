...
if __name__ == "__main__":
    if not os.path.isdir("data/Twitter10"):
        os.makedirs("data/Twitter10")
    ...
    with open("data/Twitter10/relevant_authors.json", "w") as file:
    ...
    for i, e in enumerate(G.edges):
        ...
        if i % 1000 == 0:
            print(i, "/", len(G.edges))
    print("Save nx Graph")
    nx.write_gpickle(G, "data/Twitter10/G.pickle")
    ...
    RNG = rng(G,
              distances=distances)
    nx.write_gpickle(RNG, "data/Twitter10/rng.pickle")
    ...
    GM, _, _, _, _, LP = mountainworld(RNG,
                                       heights=heights,
                                       distances=distances)
    nx.write_gpickle(GM, "data/Twitter10/GM.pickle")
    nx.write_gpickle(LP, "data/Twitter10/LP.pickle")
